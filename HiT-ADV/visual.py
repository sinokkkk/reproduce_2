...
def parse_args():
    ...
    parser.add_argument('--data_root', type=str, default='../../PC_Dataset',
                        help='directory containing modelnet40_normal_resampled')
    parser.add_argument('--sample_path', type=str, default=None,
                        help='point cloud txt file to visualize; defaults to a ModelNet car sample')
...
if __name__ == '__main__':
    ...
    sample_path = args.sample_path or os.path.join(
        args.data_root, 'modelnet40_normal_resampled', 'car', 'car_0005.txt'
    )
    if not os.path.isfile(sample_path):
        raise FileNotFoundError(
            f'Point cloud sample not found: {sample_path}. '
            'Pass --sample_path or --data_root.'
        )
    data = np.loadtxt(sample_path, delimiter=',', dtype=np.float32)
