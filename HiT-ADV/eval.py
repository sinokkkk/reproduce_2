...
def parse_args():
    ...
    parser.add_argument('--data_root', type=str, default='../../PC_Dataset',
                        help='directory containing modelnet40_normal_resampled and ShapeNetPart data')
...
if __name__ == '__main__':
    ...
    if not os.path.isfile(args.checkpoint):
        raise FileNotFoundError(
            ...
            'Download pretrained models and pass --checkpoint if needed.'
        )
    ...
    if args.dataset == 'ModelNet':
        ...
        if not os.path.isdir(data_path):
            raise FileNotFoundError(
                f'ModelNet data not found: {data_path}. '
                'Pass --data_root to the directory containing modelnet40_normal_resampled.'
            )
        test_dataset = ModelNetDataLoader(root=data_path, args=args, split='test', process_data=args.process_data)
        testDataLoader = torch.utils.data.DataLoader(test_dataset, batch_size=args.batch_size, shuffle=False,
                                                    num_workers=args.num_workers)
    elif args.dataset == 'ShapeNetPart':
        data_path = os.path.join(args.data_root, 'shapenetcore_partanno_segmentation_benchmark_v0_normal')
        if not os.path.isdir(data_path):
            raise FileNotFoundError(
                f'ShapeNetPart data not found: {data_path}. '
                'Pass --data_root to the directory containing shapenetcore_partanno_segmentation_benchmark_v0_normal.'
            )
